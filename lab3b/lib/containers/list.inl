  ...
  for (int i = 0; i < this->size_; ++i) {
    ...
    if constexpr (is_unique_ptr<T>::value) {
      ...
      if constexpr (requires(const ElementType &e) { e.copy(); }) {
        auto clonedBase = data_[i]->copy();
        // We must cast the unique_ptr<Container<...>> back to
        // unique_ptr<ElementType>
        T clonedDerived(static_cast<ElementType *>(clonedBase.release()));
        newList->append(std::move(clonedDerived));
      }
